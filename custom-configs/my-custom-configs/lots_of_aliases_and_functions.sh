...
# initializes a new virtualenv on 3.6.10
function cpinit() {
 hub clone imtapps/$1
 cd $1
 deactivate; pyenv virtualenv 3.6.10 $(basename $PWD); pyenv activate $(basename $PWD); pip install --upgrade pip && pip install -r requirements/dev.txt
}

# use like "ceinit claims-frontend"
# clones a git repo prefixed with imtapps/
# initializes ember stuff, including permissions
function ceinit() {
    hub clone imtapps/$1
    cd $1
    npm install
...
# fixes vector after pip installed requirements
function fixvector() {
  cd .
  source deactivate
  source activate $(basename $PWD)
